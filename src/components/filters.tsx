...
function Filters({ searchParams }: { searchParams: any }) {
  ...
  const onFinish = (values: any) => {
    ...
    Object.keys(values).forEach((key) => {
      if (values[key]) {
        formatedData[key] = values[key];
      }
    });

    //construct query string
    const queryString = new URLSearchParams(formatedData).toString();
      ...
      <div className=" flex justify-between p-5 border rounded-sm border-solid border-gray-300 mb-5 items-center mt-5">
        <div>
          {Object.keys(searchParams).length === 0 ? (
            <span className="text-gray-600 text-sm"> No hay filtros</span>
          ) : (
              ...
              {Object.keys(searchParams).map((key) => {
                return (
                  <div className=" capitalize flex flex-col gap-1" key={key}>
                    ...
                    <Tag
                      onClose={() => {
                        //remove the tag from the seachparams
